...
while line:
    ...
    if posTagger and eventTagger:
        eventss = eventTagger.TagSentence(words, [p.split(':')[0] for p in pos])
        events = [e.split(':')[0] for e in eventss]
    else:
        events = None
    ...
    for i in range(len(features.entities)):
        type = None
        entities.append("in="+features.entityStrings[i].split(' ', 1)[0])
...
###############################################################################################################################################
    ...
    featuresE = LdaFeatures(words, eventss)

    for i in range(len(featuresE.entities)):
        type = None
        wids = [str(vocab.GetID(x.lower())) for x in entities if vocab.HasWord(x.lower())]
        if llda and len(wids) > 0:
            entityid = "-1"
            j = 1
            labelsE = []
            for j in range(len(entities)):
                labels = []
                labels.extend(dictionaries.GetDictVector(entities[j][3:]))
            
                if sum(labels) == 0:
                    labels = [1 for x in labels]
                llda.stdin.write("\t".join([entityid, " ".join(wids), " ".join([str(x) for x in labels])]) + "\n")	
                sample = llda.stdout.readline().rstrip('\n').encode('utf8')							#z
                labelsE.extend([dict2label[dictMap[int(x)]] for x in sample[4:len(sample)-8].split(' ')])
            ...
            if maxL != 'None':
                eventss[featuresE.entities[i][0]] = "B-EVENT%s" % (maxL)
                for j in range(featuresE.entities[i][0]+1,featuresE.entities[i][1]):
                    eventss[j] = "I-EVENT%s" % (maxL)
            else:
                eventss[featuresE.entities[i][0]] = "O"
                for j in range(featuresE.entities[i][0]+1,featuresE.entities[i][1]):
                    eventss[j] = "O"
        else:
            eventss[featuresE.entities[i][0]] = "B-EVENT"
            for j in range(featuresE.entities[i][0]+1,featuresE.entities[i][1]):
                eventss[j] = "I-EVENT"
...
###############################################################################################################################################
    ...
    if eventss:
        output = ["%s/%s" % (output[x], eventss[x]) for x in range(len(output))]
