...
public class NamedEntityWrapper {
	...
	try {
		...
		System.err
			.println("environment variable TWITTER_NLP must be set");
		System.exit(-1);
	    }
	    // builder.command(System.getenv("TWITTER_NLP") +
	    // "/python/ner/extractEntities2.py");
	    builder.command(System.getenv("TWITTER_NLP")
		    + "/python/ner/extractEntities2.py", "--classify", "--pos",
		    "--event");
	    // builder.command(System.getenv("TWITTER_NLP") +
	    // "/python/ner/test.py");
	    this.p = builder.start();
	    this.stdInput = new BufferedReader(new InputStreamReader(
		    p.getInputStream()));
	    this.stdOutput = new BufferedWriter(new OutputStreamWriter(
		    p.getOutputStream()));
