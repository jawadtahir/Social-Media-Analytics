...
public class NerUsageExample {
  ...
  public static void main(String[] args) {
    ...
    NerTokenStream stream = (NerTokenStream) tokenizer.getDefaultTokenStream();
    ...
    for (String tweet : tweets) {
      ...
      while (stream.incrementToken()) {
        ...
        POSAttribute posAttribute = stream.getAttribute(POSAttribute.class);
        System.out.println(String.format("%s\t%s\t%s", nerAttribute.getToken(), nerAttribute.getType().name, posAttribute.getType().name));
        tokenCnt++;
      }
      System.out.println("");
